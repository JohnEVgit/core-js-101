...
const encodeToRot13 = (str) => {
  ...
  const keyStr = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str.split('').map((item) => ((startStr.indexOf(item) + 1) ? keyStr[startStr.indexOf(item)] : item)).join('');
};
